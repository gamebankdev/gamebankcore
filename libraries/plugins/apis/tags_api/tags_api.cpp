...
{
   public:
      tags_api_impl() : _db( appbase::app().get_plugin< gamebank::plugins::chain::chain_plugin >().db() ) {}
...
DEFINE_API_IMPL( tags_api_impl, get_discussions_by_promoted )
{
   ...
   auto tidx_itr = tidx.lower_bound( boost::make_tuple( tag, parent, share_type( GAMEBANK_MAX_SHARE_SUPPLY ) )  );
...
void tags_api_impl::set_pending_payout( discussion& d )
{
   ...
   if( itr != cidx.end() && itr->comment == d.id )  {
      d.promoted = asset( itr->promoted_balance, GBD_SYMBOL );
   }
                                                        ...
                                                        )
{
   ...
   while( count > 0 && tidx_itr != tidx.end() )
   {
      ...
      try
      {
         ...
         result.discussions.back().promoted = asset(tidx_itr->promoted_balance, GBD_SYMBOL );
...
{
   JSON_RPC_REGISTER_API( GAMEBANK_TAGS_API_PLUGIN_NAME );
}
...
{
   auto follow_api_plugin = appbase::app().find_plugin< gamebank::plugins::follow::follow_api_plugin >();
