...
#include <gamebank/plugins/account_history_api/account_history_api_plugin.hpp>
#include <gamebank/plugins/account_by_key_api/account_by_key_api_plugin.hpp>
#include <gamebank/plugins/network_broadcast_api/network_broadcast_api_plugin.hpp>
#include <gamebank/plugins/tags_api/tags_api_plugin.hpp>
#include <gamebank/plugins/follow_api/follow_api_plugin.hpp>
#include <gamebank/plugins/reputation_api/reputation_api_plugin.hpp>
...
namespace detail
{
   ...
   class condenser_api_impl
   {
      public:
         condenser_api_impl() :
            _chain( appbase::app().get_plugin< gamebank::plugins::chain::chain_plugin >() ),
            _db( _chain.db() )
         {
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_version )
   {
      ...
      (
         fc::string( GAMEBANK_BLOCKCHAIN_VERSION ),
         fc::string( gamebank::utilities::git_revision_sha ),
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_state )
   {
      ...
      try
      {
         ...
         {
			vector< fc::variant >  _vec = { fc::variant(""), fc::variant(100) };
			auto wits = get_witnesses_by_vote(_vec);
   ...
   {
      FC_ASSERT( false, "condenser_api::get_account_references --- Needs to be refactored." );
   }
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_vesting_delegations )
   {
      ...
      account_name_type account = args[0].as< account_name_type >();
	  vector< variant >		_vec = { args[0], args[1] };
      a.start = fc::variant( _vec );
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_expiring_vesting_delegations )
   {
      ...
      account_name_type account = args[0].as< account_name_type >();
	  vector< variant > _vec = { args[0], args[1], fc::variant(vesting_delegation_expiration_id_type()) };
      a.start = fc::variant( _vec );
      a.limit = args.size() == 3 ? args[2].as< uint32_t >() : 100;
   ...
   {
      CHECK_ARG_SIZE( 1 )
      auto requests = _database_api->find_gbd_conversion_requests(
         {
   ...
   DEFINE_API_IMPL(condenser_api_impl, get_nonfungible_funds)
   {
	   CHECK_ARG_SIZE(1)
		   account_name_type owner = args[0].as< account_name_type >();

	   vector< api_nonfungible_fund_object > result;
	   const auto& idx = _db.get_index< nonfungible_fund_index, by_nonfungible_fund_owner >();
	   auto itr = idx.lower_bound(owner);
   ...
   DEFINE_API_IMPL(condenser_api_impl, get_nonfungible_funds_on_sale_by_seller)
   {
       CHECK_ARG_SIZE(1)
        
       account_name_type seller = args[0].as< account_name_type >();

       vector< api_nonfungible_fund_on_sale_object > result;

       const auto& idx = _db.get_index< nonfungible_fund_on_sale_index, by_account >();
       auto itr = idx.lower_bound(seller);

       while (itr != idx.end() && itr->seller == seller)
       {
           result.push_back(*itr);
           ++itr;
       }
   ...
   DEFINE_API_IMPL(condenser_api_impl, get_nonfungible_fund_on_sale_by_fund_id)
   {
       CHECK_ARG_SIZE(1)
        
       int64_t fund_id = args[0].as< int64_t >();

       vector< api_nonfungible_fund_on_sale_object > result;

       try 
       {
           const auto& on_sale_obj =      _db.get< nonfungible_fund_on_sale_object, by_fund_id >(fund_id);

           result.push_back(on_sale_obj);
       } 
       catch ( ... ) 
       {
   ...
   #define LIMIT_GET_NONFUNGIBLE_FUNDS_ON_SALE_BY(field, begin_method, end_method) do {                                                   \
       FC_ASSERT( args.size() == 0 || args.size() == 1, "Expected 0-1 arguments, was ${n}", ("n", args.size()) ); \
       uint32_t limit = args.size() == 1 ? args[0].as< uint32_t >() : 1000;                                                               \
       limit = limit <= 1000 ? limit : 1000;                                                                                              \
       vector< api_nonfungible_fund_on_sale_object > result;                                                                              \
       const auto& idx = _db.get_index< nonfungible_fund_on_sale_index, by_##field >();                                                   \
       auto itr = idx.begin_method();                                                                                                     \
       while (itr != idx.end_method() && limit > 0) {                                                                                     \
           result.push_back(*itr);                                                                                                        \
           ++itr;                                                                                                                         \
           limit--;                                                                                                                       \
       }                                                                                                                                  \
   ...
   DEFINE_API_IMPL(condenser_api_impl, get_nonfungible_funds_on_sale)
   {
   /*
       FC_ASSERT( args.size() == 0 || args.size() == 1, "Expected 0-1 arguments, was ${n}", ("n", args.size()) );
       ...
       const auto& idx = _db.get_index< nonfungible_fund_on_sale_index, by_id >();
       auto itr = idx.rbegin();

       while (itr != idx.rend() && limit > 0)
       {
           result.push_back(*itr);
           ++itr;
           limit--;
       ...
       return result;
   */
       LIMIT_GET_NONFUNGIBLE_FUNDS_ON_SALE_BY(id, rbegin, rend);
   }

   DEFINE_API_IMPL(condenser_api_impl, get_nonfungible_funds_on_sale_by_expiration)
   {
   /*
       FC_ASSERT( args.size() == 0 || args.size() == 1, "Expected 0-1 arguments, was ${n}", ("n", args.size()) );
       ...
       const auto& idx = _db.get_index< nonfungible_fund_on_sale_index, by_expiration >();
       auto itr = idx.begin();
   ...
   */
   ...
   void condenser_api_impl::set_pending_payout( discussion& d )
   {
      if( _tags_api )
      {
         ...
         {
            d.promoted = legacy_asset::from_asset( asset( itr->promoted_balance, GBD_SYMBOL ) );
         }
      }
...
{
   JSON_RPC_REGISTER_API( GAMEBANK_CONDENSER_API_PLUGIN_NAME );
}
