...
class database_api_impl
{
   public:
      ...
      (
         ...
         (list_gbd_conversion_requests)
         (find_gbd_conversion_requests)
...
{
   JSON_RPC_REGISTER_API( GAMEBANK_DATABASE_API_PLUGIN_NAME );
}
...
{
   return gamebank::protocol::get_config();
}
...
/* GBD Conversion Requests */

DEFINE_API_IMPL( database_api_impl, list_gbd_conversion_requests )
{
   FC_ASSERT( args.limit <= DATABASE_API_SINGLE_QUERY_LIMIT );

   list_gbd_conversion_requests_return result;
   result.requests.reserve( args.limit );
...
DEFINE_API_IMPL( database_api_impl, find_gbd_conversion_requests )
{
   find_gbd_conversion_requests_return result;
...
DEFINE_API_IMPL( database_api_impl, get_order_book )
{
   ...
   auto max_sell = price::max( GBD_SYMBOL, GBC_SYMBOL );
   auto max_buy = price::max( GBC_SYMBOL, GBD_SYMBOL );
   ...
   while( sell_itr != end && sell_itr->sell_price.base.symbol == GBD_SYMBOL && result.bids.size() < args.limit )
   {
      auto itr = sell_itr;
      order cur;
      ...
      cur.gbc = ( asset( itr->for_sale, GBD_SYMBOL ) * cur.order_price ).amount;
      cur.created = itr->created;
      result.bids.push_back( cur );
      ++sell_itr;
   }
   while( buy_itr != end && buy_itr->sell_price.base.symbol == GBC_SYMBOL && result.asks.size() < args.limit )
   {
      auto itr = buy_itr;
      order cur;
      cur.order_price = itr->sell_price;
      ...
      cur.gbd     = ( asset( itr->for_sale, GBC_SYMBOL ) * cur.order_price ).amount;
      cur.created = itr->created;
...
DEFINE_API_IMPL( database_api_impl, verify_signatures )
{
   ...
   try
   {
      gamebank::protocol::verify_authority< verify_signatures_args >(
         { args },
         sig_keys,
