...
#include <gamebank/utilities/benchmark_dumper.hpp>
...
using namespace gamebank;
using fc::flat_map;
using gamebank::chain::block_id_type;
...
struct confirm_block_request
{
    confirm_block_request(const signed_block &b) :
        block(b) {}

    signed_block block;
};

typedef fc::static_variant< const signed_block*, const signed_transaction*, generate_block_request*, confirm_block_request* > write_request_ptr;
typedef fc::static_variant< boost::promise< void >*, fc::future< void >* > promise_ptr;

//boost::lockfree::queue< write_context* >的元素
struct write_context
{
   ...
   promise_ptr                   prom_ptr;	//static_variant()
};
...
class chain_plugin_impl
{
   public:
      ...
      bool                             running = true;
      std::shared_ptr< std::thread >   write_processor_thread;	//写线程，往db写？
      boost::lockfree::queue< write_context* > write_queue;		//无锁队列，往队列写数据的调用者应该确保write_context指针在结果出来前有效
...
//执行任务
struct write_request_visitor			//函数对象, as a vistor
{
...
// 生成区块
   ...
   bool operator()( confirm_block_request* req )
   {
       bool result = false;

       try
       {
           STATSD_START_TIMER(chain, write_time, confirm_block, 1.0f)
           db->confirm_block(req->block);
           STATSD_STOP_TIMER(chain, write_time, confirm_block)
       ...
       {
           *except = fc::unhandled_exception(FC_LOG_MESSAGE(warn, "Unexpected exception while pushing confirm block."),
               std::current_exception());
       }
...
void chain_plugin_impl::start_write_processing()
{
   ...
   //线程函数
      ...
      write_request_visitor req_visitor;	//可调用对象，定义了写入交易、写入区块、创建新区块的操作
      req_visitor.db = &db;					

      request_promise_visitor prom_visitor;	//可调用对象

		//循环检测write_queue并写入db
		//
	  ...
	  //also @see accept_block/accept_trascation/generate_block_request, them push ctx into write_queue
      while( running )
      {
         if( !is_syncing )
         ...
         if( write_queue.pop( cxt ) )
         {
            db.with_write_lock( [&]()
            {
               ...
               while( true )
               {
				  ...
				  //core!!
				  //maybe run write_request_visitor(signed_block)
				  //执行任务队列中的任务
                  cxt->success = cxt->req_ptr.visit( req_visitor );	//最后会执行req_visitor()
                  cxt->prom_ptr.visit( prom_visitor );				////最后会执行prom_visitor()
				  ...
				  //live模式且持有锁的时间超时，退出内层循环后sleep 10ms
         ...
         if( !is_syncing )
            boost::this_thread::sleep_for( boost::chrono::milliseconds( 10 ) );	//Live mode only
      }
   });
}
...
const gamebank::chain::database& chain_plugin::db() const { return my->db; }
...
#ifdef IS_TEST_NET
         ("chain-id", bpo::value< std::string >()->default_value( GAMEBANK_CHAIN_ID_NAME ), "chain ID to connect to")
#endif
         ;
}
...
void chain_plugin::plugin_startup()
{
   ...
   {
	//查找statsd_plugin是否已注册
      auto statsd = appbase::app().find_plugin< gamebank::plugins::statsd::statsd_plugin >();
      if( statsd != nullptr )
      {
   ...
   //启动写线程，将收到的申请（接收新区块、新交易、创建区块）写入db
   ...
   gamebank::utilities::benchmark_dumper dumper;

   const auto& abstract_index_cntr = my->db.get_abstract_index_cntr();

   typedef gamebank::utilities::benchmark_dumper::index_memory_details_cntr_t index_memory_details_cntr_t;
   ...
   db_open_args.initial_supply = GAMEBANK_INIT_SUPPLY;
      ...
      const chainbase::database::abstract_index_cntr_t& abstract_index_cntr )
   {
      ...
      {
         typedef gamebank::utilities::benchmark_dumper::database_object_sizeof_cntr_t database_object_sizeof_cntr_t;
      ...
      const gamebank::utilities::benchmark_dumper::measurement& measure =
   ...
   if(my->replay)
   {
      ...
      db_open_args.benchmark = gamebank::chain::database::TBenchmark(my->benchmark_interval, benchmark_lambda);
      last_block_number = my->db.reindex( db_open_args );

      if( my->benchmark_interval > 0 )
      {
         const gamebank::utilities::benchmark_dumper::measurement& total_data = dumper.dump(true, get_indexes_memory_details);
   ...
   {
      db_open_args.benchmark = gamebank::chain::database::TBenchmark(dump_memory_details, benchmark_lambda);
      ...
      catch( const fc::exception& e )
      {
         ...
         catch( gamebank::chain::block_log_exception& )
         {
...
//生产者accept_block/accept_transaction/generate_block写入write_context到write_queue
//消费者线程start_write_processing处理请求，从write_queue取下并写入db
//accept_block called by p2p_plugin
bool chain_plugin::accept_block( const gamebank::chain::signed_block& block, bool currently_syncing, uint32_t skip )
{
...
bool chain_plugin::accept_confirm( const gamebank::chain::signed_block& block )
{
    check_time_in_block(block);

    confirm_block_request req(block);
    boost::promise< void > prom;
    write_context cxt;
    cxt.req_ptr = &req;
    cxt.prom_ptr = &prom;
...
//接收网络来的新交易
void chain_plugin::accept_transaction( const gamebank::chain::signed_transaction& trx )
{
   boost::promise< void > prom;
   write_context cxt;
   cxt.req_ptr = &trx;
...
//区块生产处理
gamebank::chain::signed_block chain_plugin::generate_block(
   const fc::time_point_sec when,
   const account_name_type& witness_owner,
   const fc::ecc::private_key& block_signing_private_key,
   uint32_t skip )
...
//判断一个block是在最优链还是分叉链?
bool chain_plugin::block_is_on_preferred_chain(const gamebank::chain::block_id_type& block_id )
{
   ...
   return db().get_block_id_for_num( gamebank::chain::block_header::num_from_id( block_id ) ) == block_id;
}

void chain_plugin::check_time_in_block( const gamebank::chain::signed_block& block )
{
...
} } } // namespace gamebank::plugis::chain::chain_apis
