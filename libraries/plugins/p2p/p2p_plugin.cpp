...
#include <gamebank/plugins/p2p/p2p_default_seeds.hpp>
...
using gamebank::protocol::block_header;
using gamebank::protocol::signed_block_header;
using gamebank::protocol::signed_block;
using gamebank::protocol::block_id_type;
...
public:
   ...
   virtual gamebank::protocol::chain_id_type get_chain_id() const override;

   // node_delegate interface
   virtual bool has_item( const graphene::net::item_id& ) override;
   virtual bool handle_confirm(const graphene::net::confirm_message&) override;
   ...
   fc::optional<fc::ip::endpoint> endpoint;     //The local
   vector<fc::ip::endpoint> seeds;              //配置的的seed_nodes
   string user_agent;                           
   fc::mutable_variant_object config;
   uint32_t max_connections = 0;                //依据配置设定的p2p-max-connections
...
void p2p_plugin_impl::on_post_apply_block( const signed_block& b)
{
    ulog("Apply Block Broadcasting confirm #${n}", ("n", b.block_num()));
    node->broadcast(graphene::net::confirm_message(b));
}
...
bool p2p_plugin_impl::has_item( const graphene::net::item_id& id )
{
   return chain.db().with_read_lock( [&]()
   {
      try
      {
         if( id.item_type == graphene::net::block_message_type || id.item_type == graphene::net::confirm_message_type )
...
bool p2p_plugin_impl::handle_confirm( const graphene::net::confirm_message& cfm_msg )
{ try {
   if (running.load())
   {
      ...
      try {
         ...
         bool result = chain.accept_confirm( cfm_msg.block );

         fc::microseconds latency = fc::time_point::now() - cfm_msg.block.timestamp;
         ilog("Got ${t} transactions on comfirm block ${b} by ${w} -- latency: ${l} ms",
             ("t", cfm_msg.block.transactions.size())
             ("b", cfm_msg.block.block_num())
             ("w", cfm_msg.block.witness)
    ...
    {
        ilog("Confirm ignored due to started p2p_plugin shutdown");
        if (handleBlockFinished.second.valid() == false)
            handleBlockFinished.first.set_value();
        FC_THROW("Preventing further processing of ignored confirm...");
    }
    return false;

} FC_CAPTURE_AND_RETHROW( (cfm_msg) ) }
...
gamebank::protocol::chain_id_type p2p_plugin_impl::get_chain_id() const
{
...
void p2p_plugin::plugin_initialize(const boost::program_options::variables_map& options)
{
   ...
   my->user_agent = "Gamebank Reference Implementation";
   ...
   my->_post_apply_block_conn = my->chain.db().add_post_apply_block_handler(
       [&](const chain::block_notification& note) { my->on_post_apply_block(note.block); }, *this, 0);
}
...
void p2p_plugin::broadcast_block( const gamebank::protocol::signed_block& block )
{
   ulog("Broadcasting block #${n}", ("n", block.block_num()));
   my->node->broadcast( graphene::net::block_message( block ) );
}

void p2p_plugin::broadcast_transaction( const gamebank::protocol::signed_transaction& tx )
{
   ulog("Broadcasting tx #${n}", ("id", tx.id()));
   my->node->broadcast( graphene::net::trx_message( tx ) );
}

void p2p_plugin::broadcast_confirm( const gamebank::protocol::signed_block& block )
{
    ulog("Broadcasting confirm #${n}", ("n", block.block_num()));
    my->node->broadcast( graphene::net::confirm_message( block ) );
}
...
} } } // namespace gamebank::plugins::p2p
