#include <gamebank/utilities/git_revision.hpp>
#include <gamebank/utilities/key_conversion.hpp>
#include <gamebank/utilities/words.hpp>

#include <gamebank/protocol/base.hpp>
#include <gamebank/wallet/wallet.hpp>
#include <gamebank/wallet/api_documentation.hpp>
#include <gamebank/wallet/reflect_util.hpp>
#include <gamebank/wallet/remote_node_api.hpp>
...
namespace gamebank { namespace wallet {

using gamebank::plugins::condenser_api::legacy_asset;
...
public:
   wallet_api& self;
   wallet_api_impl( wallet_api& s, const wallet_data& initial_data, const gamebank::protocol::chain_id_type& _gamebank_chain_id, fc::api< remote_node_api > rapi )
      : self( s ),
        _remote_api( rapi )
   {
      init_prototype_ops();
      ...
      gamebank_chain_id = _gamebank_chain_id;
   }
   ...
   variant info() const
   {
      ...
      result["median_gbd_price"] = _remote_api->get_current_median_history_price();
      result["account_creation_fee"] = _remote_api->get_chain_properties().account_creation_fee;
      result["post_reward_fund"] = fc::variant(_remote_api->get_reward_fund( GAMEBANK_POST_REWARD_FUND_NAME )).get_object();
      return result;
   }

   variant_object about() const
   {
      string client_version( gamebank::utilities::git_revision_description );
      const size_t pos = client_version.find( '/' );
      if( pos != string::npos && client_version.size() > pos )
         client_version = client_version.substr( pos + 1 );

      fc::mutable_variant_object result;
      result["blockchain_version"]       = GAMEBANK_BLOCKCHAIN_VERSION;
      result["client_version"]           = client_version;
      result["gamebank_revision"]           = gamebank::utilities::git_revision_sha;
      result["gamebank_revision_age"]       = fc::get_approximate_relative_time_string( fc::time_point_sec( gamebank::utilities::git_revision_unix_timestamp ) );
      ...
      try
      {
         ...
         result["server_gamebank_revision"] = v.gamebank_revision;
   ...
   bool import_key(string wif_key)
   {
      ...
      gamebank::chain::public_key_type wif_pub_key = optional_private_key->get_public_key();
   ...
   int find_first_unused_derived_key_index(const fc::ecc::private_key& parent_key)
   {
      ...
      for (int key_index = 0; ; ++key_index)
      {
         ...
         gamebank::chain::public_key_type derived_public_key = derived_private_key.get_public_key();
   ...
   { try {
         ...
         gamebank::chain::public_key_type owner_pubkey = owner_privkey.get_public_key();
         gamebank::chain::public_key_type active_pubkey = active_privkey.get_public_key();
         gamebank::chain::public_key_type memo_pubkey = memo_privkey.get_public_key();
         ...
         account_create_op.memo_key = memo_pubkey;		//远端节点可用的pub key
   ...
   {
      FC_ASSERT( tx_expiration_seconds < GAMEBANK_MAX_TIME_UNTIL_EXPIRATION );
...
//1 处理交易中所有操作需要的账户授权
      ...
      //本次交易需要的相关账户的active，owner，post授权
      //req_active_approvals： 提供active授权的账户
      //req_owner_approvals：提供owner授权的账户
      //req_posting_approvals：提供post授权的账户
...
//2 打包所有提供授权的账户
      ...
      //   at the same time
      //三种类型的账户合并到v_approving_account_names列表
...
//3 建立<账户名，api_account_object>映射表
	  ...
	  //存放所有账户(包括avtive,post,owner权限的)pub key
...
//7 使用有效的公钥-获取对应的私钥-对交易签名
      ...
      for( const public_key_type& k : minimal_signing_keys )
      {
		 ...
		 //然后根据序列化后的[交易摘要]生成签名数据
         tx.sign( it->second, gamebank_chain_id );
      }
...
//8 广播交易
   ...
   std::map<string,std::function<string(fc::variant,const fc::variants&)>> get_result_formatters() const
   {
      ...
      m["get_open_orders"] = []( variant result, const fc::variants& a ) {
          ...
          for( const auto& o : orders ) {
             ...
             ss << ' ' << setw( 10 ) << (o.sell_price.base.symbol == GBC_SYMBOL ? "SELL" : "BUY");
             ss << "\n";
          }
          return ss.str();
      };
      m["get_order_book"] = []( variant result, const fc::variants& a ) {
         ...
         std::stringstream ss;
         asset bid_sum = asset( 0, GBD_SYMBOL );
         asset ask_sum = asset( 0, GBD_SYMBOL );
         int spacing = 24;
         ...
         ss << ' ' << setw( ( spacing * 4 ) + 6 ) << "Bids" << "Asks\n"
            << ' '
            << setw( spacing + 3 ) << "Sum(GBD)"
            << setw( spacing + 1) << "GBD"
            << setw( spacing + 1 ) << "GBC"
            << setw( spacing + 1 ) << "Price"
            << setw( spacing + 1 ) << "Price"
            << setw( spacing + 1 ) << "GBC "
            << setw( spacing + 1 ) << "GBD " << "Sum(GBD)"
            << "\n====================================================================================================="
            << "|=====================================================================================================\n";
         ...
         for( size_t i = 0; i < orders.bids.size() || i < orders.asks.size(); i++ )
         {
            ...
            {
               bid_sum += asset( orders.bids[i].gbd, GBD_SYMBOL );
               ss
                  << ' ' << setw( spacing ) << legacy_asset::from_asset( bid_sum ).to_string()
                  << ' ' << setw( spacing ) << legacy_asset::from_asset( asset( orders.bids[i].gbd, GBD_SYMBOL ) ).to_string()
                  << ' ' << setw( spacing ) << legacy_asset::from_asset( asset( orders.bids[i].gbc, GBC_SYMBOL ) ).to_string()
            ...
            {
               ask_sum += asset( orders.asks[i].gbd, GBD_SYMBOL );
               ss << ' ' << setw( spacing ) << orders.asks[i].real_price
                  << ' ' << setw( spacing ) << legacy_asset::from_asset( asset( orders.asks[i].gbc, GBC_SYMBOL ) ).to_string()
                  << ' ' << setw( spacing ) << legacy_asset::from_asset( asset( orders.asks[i].gbd, GBD_SYMBOL ) ).to_string()
   ...
   wallet_data                             _wallet;
   gamebank::protocol::chain_id_type          gamebank_chain_id;

   map<public_key_type,string>             _keys;		//see import_key
   fc::sha512                              _checksum;	//加密后的钱包密码
   fc::api< remote_node_api >              _remote_api;
   uint32_t                                _tx_expiration_seconds = 30;

   flat_map<string, operation>             _prototype_ops;	//for get_prototype_operation
...
} } } // gamebank::wallet::detail



namespace gamebank { namespace wallet {

wallet_api::wallet_api(const wallet_data& initial_data, const gamebank::protocol::chain_id_type& _gamebank_chain_id, fc::api< remote_node_api > rapi)
   : my(new detail::wallet_api_impl(*this, initial_data, _gamebank_chain_id, rapi))
{}
...
brain_key_info wallet_api::suggest_brain_key()const
{
   ...
   {
      fc::bigint choice = entropy % gamebank::words::word_list_size;
      entropy /= gamebank::words::word_list_size;
      if( i > 0 )
         brain_key += " ";
      brain_key += gamebank::words::word_list[ choice.to_int64() ];
   }
...
{ try {
   ...
   //新账户的默认权限配置
   op.owner = authority( 1, owner, 1 );			//onwer权限，公钥owner，权重1，阈值1; 使用owner权限时只需要输入公钥
   op.active = authority( 1, active, 1 );		//active权限，公钥active，权重1，阈值1; 使用active权限时只需要输入公钥
   op.posting = authority( 1, posting, 1 );		//post权限，公钥post，权重1，阈值1; 使用post权限时只需要输入公钥
   op.memo_key = memo;
...
condenser_api::legacy_signed_transaction wallet_api::create_account_with_keys_delegated(
   string creator,
   condenser_api::legacy_asset gb_fee,
...
{ try {
   ...
   op.memo_key = memo;
   op.json_metadata = json_meta;
   op.fee = gb_fee.to_asset(); //新账号接受op.fee，创建者balance -= op.fee
   op.delegation = delegated_vests.to_asset(); //新账号的received_vesting_shares = op.delegation;创建者delegated_vesting_shares += op.delegation;
...
condenser_api::legacy_signed_transaction wallet_api::update_account_auth_key(
   string account_name,
   authority_type type,			//要修改的权限类型(owner或active或posting)
   public_key_type key,			//权限需要的公钥
   weight_type weight,			//公钥的权重
   bool broadcast )
{
   FC_ASSERT( !is_locked() );
...
condenser_api::legacy_signed_transaction wallet_api::update_account_auth_account(
   string account_name,			//给定账户
   authority_type type,			//权限类型
   string auth_account,			
   weight_type weight,			//账户权重
   bool broadcast )
{
   FC_ASSERT( !is_locked() );
   ...
   {
      //为权限增加账户及其对应的权重
      //auth_account存在：修改auth_account的权重?
      //auth_account不存在：添加新的账户及其权重？
      new_auth.add_authority( auth_account, weight );
   }

   //配置新的账户权重后，验证是否满足权重总和>=权限阈值
   if( new_auth.is_impossible() )
   {
      if ( type == owner )
      {
...
{ try {
   ...
   import_key( posting.wif_priv_key );
   import_key( memo.wif_priv_key );
   return create_account_with_keys_delegated( creator, gb_fee, delegated_vests, new_account_name, json_meta,  owner.pub_key, active.pub_key, posting.pub_key, memo.pub_key, broadcast );
} FC_CAPTURE_AND_RETHROW( (creator)(new_account_name)(json_meta) ) }
...
condenser_api::legacy_signed_transaction wallet_api::escrow_transfer(
   ...
   string agent,
   uint32_t escrow_id,
   condenser_api::legacy_asset gbd_amount,
   condenser_api::legacy_asset gbc_amount,
   ...
   bool broadcast )
{
   ...
   op.gbd_amount = gbd_amount.to_asset();
   op.gbc_amount = gbc_amount.to_asset();
   op.fee = fee.to_asset();
   ...
   bool broadcast )
{
   ...
   //没有赋值approve？？
   //escrow_approve_operation::approve默认为true
   op.from = from;
   op.to = to;
   op.agent = agent;
   op.who = who;
...
condenser_api::legacy_signed_transaction wallet_api::escrow_release(
   ...
   string to,
   string agent,
   string who,					//负责release资金的账户
   string receiver,				//资金最终归属
   uint32_t escrow_id,
   condenser_api::legacy_asset gbd_amount,//release的GBD资金量（可能小于发起额，则多次release）
   condenser_api::legacy_asset gbc_amount,//release的GBC资金量，可能小于发起额，则多次release)
   bool broadcast )
{
   FC_ASSERT( !is_locked() );
...
condenser_api::legacy_signed_transaction wallet_api::convert_gbd(
   string from,
   condenser_api::legacy_asset amount,
   bool broadcast )
{
...
condenser_api::legacy_signed_transaction wallet_api::claim_reward_balance(
   string account,
   condenser_api::legacy_asset reward_gbc,
   condenser_api::legacy_asset reward_gbd,
   ...
   bool broadcast )
{
   ...
   op.account = account;
   op.reward_gbc = reward_gbc.to_asset();
   op.reward_gbd = reward_gbd.to_asset();
...
condenser_api::legacy_signed_transaction wallet_api::create_order(
   ...
   bool fill_or_kill,
   uint32_t expiration_sec,					//order过期时间（秒）
   bool broadcast )
{
   FC_ASSERT( !is_locked() );
   ...
   bool broadcast )
{
   ...
   op.author = author;			//发起评论的账户(需要有Post权限)
   ...
   bool broadcast )
{
   ...
   op.author = author;
   op.permlink = permlink;
   op.weight = weight * GAMEBANK_1_PERCENT;
...
condenser_api::legacy_signed_transaction wallet_api::follow( string follower, string following, set<string> what, bool broadcast )
{
   ...
   jop.required_posting_auths.insert(follower);//发起“关注”的账户需要post权限
...
condenser_api::legacy_signed_transaction wallet_api::crowdfunding( string originator, string permlink, condenser_api::legacy_asset raise, string title, string body, string json_metadata, bool broadcast )
{
    FC_ASSERT(!is_locked());

    crowdfunding_operation op;
    op.originator = originator;
    op.permlink = permlink;
    op.raise = raise.to_asset();
    op.title = title;
    op.body = body;
...
condenser_api::legacy_signed_transaction wallet_api::invest( string invester, string originator, string permlink, condenser_api::legacy_asset raise, bool broadcast )
{
    FC_ASSERT(!is_locked());

    invest_operation op;
    op.invester = invester;
    op.originator = originator;
    op.permlink = permlink;
    op.raise = raise.to_asset();
...
condenser_api::legacy_signed_transaction wallet_api::create_nonfungible_fund(
	string creator,
	string owner,
	string metadata,
	bool broadcast)
{
	FC_ASSERT(!is_locked());
	nonfungible_fund_create_operation op;
	op.creator = creator;
	op.owner = owner;
	op.meta_data = metadata;
...
condenser_api::legacy_signed_transaction wallet_api::transfer_nonfungible_fund(
	string from,
	string to,
	int64_t fund_id,
	bool broadcast)
{
	FC_ASSERT(!is_locked());
	nonfungible_fund_transfer_operation op;
	op.from = from;
	op.to = to;
	op.fund_id = fund_id;
...
vector< condenser_api::api_nonfungible_fund_object > wallet_api::get_nonfungible_funds(string owner)
{
	return my->_remote_api->get_nonfungible_funds(owner);
}

condenser_api::legacy_signed_transaction wallet_api::put_up_nonfungible_fund_for_sale(
    string seller,
    int64_t fund_id,
    condenser_api::legacy_asset sell_price,
    uint32_t expiration_sec,
    bool broadcast)
{
    FC_ASSERT( !is_locked() );
    
    nonfungible_fund_put_up_for_sale_operation op;
    
    op.seller = seller;
    op.fund_id = fund_id;
    op.selling_price = sell_price.to_asset();
    op.expiration = expiration_sec ? (fc::time_point::now() + fc::seconds(expiration_sec)) : fc::time_point::maximum();
...
condenser_api::legacy_signed_transaction wallet_api::withdraw_nonfungible_fund_from_sale(
    string   seller,
    int64_t  fund_id,
    bool     broadcast)
{
    ...
    nonfungible_fund_withdraw_from_sale_operation op;
...
condenser_api::legacy_signed_transaction wallet_api::buy_nonfungible_fund(
    string   buyer,
    int64_t  fund_id,
    bool     broadcast)
{
    ...
    nonfungible_fund_buy_operation op;
...
vector< condenser_api::api_nonfungible_fund_on_sale_object > wallet_api::get_nonfungible_funds_on_sale_by_seller(string seller)
{
    return my->_remote_api->get_nonfungible_funds_on_sale_by_seller(seller);
}
    
vector< condenser_api::api_nonfungible_fund_on_sale_object > wallet_api::get_nonfungible_fund_on_sale_by_fund_id(int64_t fund_id)
{
    return my->_remote_api->get_nonfungible_fund_on_sale_by_fund_id(fund_id);
}

vector< condenser_api::api_nonfungible_fund_on_sale_object > wallet_api::get_nonfungible_funds_on_sale( uint32_t limit )
{
    FC_ASSERT( limit <= 1000 );

    return my->_remote_api->get_nonfungible_funds_on_sale(limit);
}

vector< condenser_api::api_nonfungible_fund_on_sale_object > wallet_api::get_nonfungible_funds_on_sale_by_expiration( uint32_t limit )
{
    FC_ASSERT( limit <= 1000 );
...
condenser_api::legacy_signed_transaction wallet_api::contract_deploy(string creator, string contract_name, string code, string abi, bool broadcast)
{
    FC_ASSERT(!is_locked());
...
vector< string > wallet_api::list_contracts(uint32_t start, uint32_t limit)
{
    FC_ASSERT(!is_locked());

    FC_ASSERT(limit <= 1000);

    return my->_remote_api->list_contracts(start, limit);
}

condenser_api::api_contract_object wallet_api::find_contracts(string name)
{
    FC_ASSERT(!is_locked());

     return my->_remote_api->find_contracts(name);
}
