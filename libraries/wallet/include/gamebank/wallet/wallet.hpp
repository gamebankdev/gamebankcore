...
using namespace gamebank::utilities;
...
{
   public:
      wallet_api( const wallet_data& initial_data, const gamebank::protocol::chain_id_type& _gamebank_chain_id, fc::api< remote_node_api > rapi );
      virtual ~wallet_api();
       ...
       *  'info' wallet command.
       *
       *  These accounts are created with combination of GBC and delegated SP
       *
       *  @param creator The account creating the new account
       *  @param gb_fee The amount of the fee to be paid with GBC
      ...
      /**
       * This method delegates GBS from one account to another.
       *
       * @param delegator The name of the account delegating GBS
       * @param delegatee The name of the account receiving GBS
       * @param vesting_shares The amount of GBS to delegate
       * @param broadcast true if you wish to broadcast the transaction
       */
      ...
      /**
       * Transfer funds from one account to another. GBC and GBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param amount The funds being transferred. i.e. "100.000 GBC"
      ...
      /**
       * Transfer funds from one account to another using escrow. GBC and GBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param agent The account acting as the agent in case of dispute
       * @param escrow_id A unique id for the escrow transfer. (from, escrow_id) must be a unique pair
       * @param gbd_amount The amount of GBD to transfer
       * @param gbc_amount The amount of GBC to transfer
       ...
       * @param escrow_id A unique id for the escrow transfer
       * @param gbd_amount The amount of GBD that will be released
       * @param gbc_amount The amount of GBC that will be released
       * @param broadcast true if you wish to broadcast the transaction
       */
      ...
      /**
       * Transfer GBC into a vesting fund represented by vesting shares (GBS). GBS are required to vesting
       * for a minimum of one coin year and can be withdrawn once a week over a two year withdraw period.
       * GBS are protected against dilution up until 90% of GBC is vesting.
       *
       * @param from The account the GBC is coming from
       * @param to The account getting the GBS
       * @param amount The amount of GBC to vest i.e. "100.00 GBC"
       * @param broadcast true if you wish to broadcast the transaction
       */
       ...
       *
       * @param from The account the GBS are withdrawn from
       * @param vesting_shares The amount of GBS to withdraw over the next two years. Each week (amount/104) shares are
       *    withdrawn and deposited back as GBC. i.e. "10.000000 GBS"
       * @param broadcast true if you wish to broadcast the transaction
       */
       ...
       *
       * @param from The account the GBS are withdrawn from.
       * @param to   The account receiving either GBS or GBC.
       ...
       * @param auto_vest Set to true if the from account should receive the GBS as GBS, or false if it should receive
       *    them as GBC.
       * @param broadcast true if you wish to broadcast the transaction.
       */
      ...
      /**
       *  This method will convert GBD to GBC at the current_median_history price one
       *  week from the time it is executed. This method depends upon there being a valid price feed.
       *
       *  @param from The account requesting conversion of its GBD i.e. "1.000 GBD"
       *  @param amount The amount of GBD to convert
       *  @param broadcast true if you wish to broadcast the transaction
       */
      condenser_api::legacy_signed_transaction convert_gbd(
         string from,
         condenser_api::legacy_asset amount,
         bool broadcast = false );

      /**
       * A witness can public a price feed for the GBC:GBD market. The median price feed is used
       * to process conversion requests from GBD to GBC.
       *
       ...
       *                       operations defined in `gamebank/chain/operations.hpp`
      ...
      /**
       * Gets the current order book for GBC:GBD
       *
       ...
       *  @param amount_to_sell The amount of either GBD or GBC you wish to sell
      ...
      /**
       * Vote on a comment to be paid GBC
       *
      ...
      condenser_api::legacy_signed_transaction crowdfunding(
          string originator,
          string permlink,
          condenser_api::legacy_asset raise,
      ...
      condenser_api::legacy_signed_transaction invest(
          string invester,
          string originator,
          string permlink,
          condenser_api::legacy_asset raise,
	  ...
	  /**
	  *  Creates a nonfungible fund
	  *
	  *  @param creator The name of the account creating the nonfungible fund
	  *  @param owner who own the fund after create
	  *  @param metadata the data of the fund
	  *  @param broadcast true if you wish to broadcast the transaction
	  */
	  condenser_api::legacy_signed_transaction create_nonfungible_fund(
		  string creator,
		  string owner,
		  string metadata,
		  bool broadcast);

	  condenser_api::legacy_signed_transaction transfer_nonfungible_fund(
		  string from,
		  string to,
		  int64_t fund_id,
		  bool broadcast);

	  vector< condenser_api::api_nonfungible_fund_object > get_nonfungible_funds(string owner);

      /**
       *  put up nonfungible fund into market for sale
       *
       *  @param seller The name of the account owned the nonfungible fund
       *  @param fund_id ID of the fund to be sell
       *  @param sell_price price of the fund to be sell
       *  @param expiration the time the on-sale fund should expire if it has not been selt
       *  @param broadcast true if you wish to broadcast the transaction
       */
       condenser_api::legacy_signed_transaction put_up_nonfungible_fund_for_sale(
           string seller,
           int64_t fund_id,
       ...
       condenser_api::legacy_signed_transaction withdraw_nonfungible_fund_from_sale(
           string   seller,
           int64_t  fund_id,
           bool     broadcast);
       
       condenser_api::legacy_signed_transaction buy_nonfungible_fund(
           string   buyer,
           int64_t  fund_id,
           bool     broadcast);

       vector< condenser_api::api_nonfungible_fund_on_sale_object > get_nonfungible_funds_on_sale_by_seller(string seller);
       vector< condenser_api::api_nonfungible_fund_on_sale_object > get_nonfungible_fund_on_sale_by_fund_id(int64_t fund_id);
...
FC_REFLECT( gamebank::wallet::wallet_data,
            (cipher_keys)
            (ws_server)
            (ws_user)
            (ws_password)
...
FC_REFLECT( gamebank::wallet::brain_key_info, (brain_priv_key)(wif_priv_key) (pub_key))

FC_REFLECT( gamebank::wallet::plain_keys, (checksum)(keys) )

FC_REFLECT_ENUM( gamebank::wallet::authority_type, (owner)(active)(posting) )

FC_API( gamebank::wallet::wallet_api,
        /// wallet api
        (help)(gethelp)
        ...
        (crowdfunding)
        (invest)
	    (create_nonfungible_fund)
	    (transfer_nonfungible_fund)
	    (get_nonfungible_funds)
	    (put_up_nonfungible_fund_for_sale)
	    (withdraw_nonfungible_fund_from_sale)
	    (buy_nonfungible_fund)
...
FC_REFLECT( gamebank::wallet::memo_data, (from)(to)(nonce)(check)(encrypted) )
