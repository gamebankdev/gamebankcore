...
struct remote_node_api
{
   ...
   optional< condenser_api::legacy_signed_contract > get_contract(uint32_t);
   ...
   vector< condenser_api::api_nonfungible_fund_object > get_nonfungible_funds(account_name_type);
   vector< condenser_api::api_nonfungible_fund_on_sale_object > get_nonfungible_funds_on_sale_by_seller(account_name_type);
...
FC_API( gamebank::wallet::remote_node_api,
        (get_version)
        (get_trending_tags)
        (get_state)
