...
void reset_virtual_schedule_time( database& db )
{
   ...
   for( const auto& witness : idx )
   {
      db.modify( witness, [&]( witness_object& wobj )
      {
         ...
         wobj.virtual_scheduled_time = GAMEBANK_VIRTUAL_SCHEDULE_LAP_LENGTH2 / (wobj.votes.value+1);
      } );
   }
}
...
void update_median_witness_props( database& db )
{
   ...
   /// sort them by gbd_interest_rate
   std::sort( active.begin(), active.end(), [&]( const witness_object* a, const witness_object* b )
   {
      return a->props.gbd_interest_rate < b->props.gbd_interest_rate;
   } );
   uint16_t median_gbd_interest_rate = active[active.size()/2]->props.gbd_interest_rate;
   ...
   db.modify( wso, [&]( witness_schedule_object& _wso )
   {
      ...
      _wso.median_props.gbd_interest_rate       = median_gbd_interest_rate;
   ...
   db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& _dgpo )
   {
      ...
      _dgpo.gbd_interest_rate  = median_gbd_interest_rate;
   } );
}
...
void update_witness_schedule4( database& db )
{
   ...
   active_witnesses.reserve( GAMEBANK_MAX_WITNESSES );
   ...
   for( auto witness_count = selected_voted.size() + selected_miners.size();
        sitr != schedule_idx.end() && witness_count < GAMEBANK_MAX_WITNESSES;
        ++sitr )
   {
   ...
   {
      auto new_virtual_scheduled_time = new_virtual_time + GAMEBANK_VIRTUAL_SCHEDULE_LAP_LENGTH2 / ((*itr)->votes.value+1);
   ...
   size_t expected_active_witnesses = std::min( size_t(GAMEBANK_MAX_WITNESSES), widx.size() );
   FC_ASSERT( active_witnesses.size() == expected_active_witnesses, "number of active witnesses does not equal expected_active_witnesses=${expected_active_witnesses}",
                                       ("active_witnesses.size()",active_witnesses.size()) ("GAMEBANK_MAX_WITNESSES",GAMEBANK_MAX_WITNESSES) ("expected_active_witnesses", expected_active_witnesses) );
   ...
   db.modify( wso, [&]( witness_schedule_object& _wso )
   {
      ...
      for( size_t i = active_witnesses.size(); i < GAMEBANK_MAX_WITNESSES; i++ )
      {
...
{
   if( (db.head_block_num() % GAMEBANK_MAX_WITNESSES) == 0 ) //wso.next_shuffle_block_num )
   {
