...
template< typename OperationType=gamebank::protocol::operation >
class evaluator
{
   public:
      virtual ~evaluator() {}
...
template< typename EvaluatorType, typename OperationType=gamebank::protocol::operation >
...
#define GAMEBANK_DEFINE_EVALUATOR( X ) \
class X ## _evaluator : public gamebank::chain::evaluator_impl< X ## _evaluator > \
{                                                                           \
   public:                                                                  \
      typedef X ## _operation operation_type;                               \
                                                                            \
      X ## _evaluator( database& db )                                       \
         : gamebank::chain::evaluator_impl< X ## _evaluator >( db )          \
      {}                                                                    \
                                                                            \
      void do_apply( const X ## _operation& o );                            \
};

#define GAMEBANK_DEFINE_PLUGIN_EVALUATOR( PLUGIN, OPERATION, X )               \
class X ## _evaluator : public gamebank::chain::evaluator_impl< X ## _evaluator, OPERATION > \
{                                                                           \
   public:                                                                  \
      typedef X ## _operation operation_type;                               \
                                                                            \
      X ## _evaluator( gamebank::chain::database& db, PLUGIN* plugin )       \
         : gamebank::chain::evaluator_impl< X ## _evaluator, OPERATION >( db ), \
           _plugin( plugin )                                                \
      {}                                                                    \
                                                                            \
      void do_apply( const X ## _operation& o );                            \
