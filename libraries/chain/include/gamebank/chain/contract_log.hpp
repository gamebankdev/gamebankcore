...
namespace gamebank { namespace chain {
   ...
   namespace detail { class contract_log_impl; }
   ...
   class contract_log {
      public:
         ...
         uint64_t append( const signed_contract& b );
         void flush();
         std::pair< signed_contract, uint64_t > read_block( uint64_t file_pos )const;
         optional< signed_contract > read_block_by_num( uint32_t block_num )const;

         uint64_t get_block_pos( uint32_t block_num ) const;
         signed_contract read_head()const;
         const optional< signed_contract >& head()const;
      ...
      private:
         ...
         std::pair< signed_contract, uint64_t > read_block_helper( uint64_t file_pos )const;
         uint64_t get_block_pos_helper( uint32_t block_num ) const;

         std::unique_ptr<detail::contract_log_impl> my;
   };
