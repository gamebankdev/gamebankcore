...
#include <gamebank/chain/hardfork_property_object.hpp>
...
namespace gamebank { namespace chain {
   ...
   using gamebank::protocol::operation;
   ...
   class database : public chainbase::database
   {
      public:
         ...
         struct open_args
         {
            ...
            uint64_t initial_supply = GAMEBANK_INIT_SUPPLY;
         ...
         optional<signed_contract>  fetch_contract_by_id(const block_id_type& id)const;
         optional<signed_contract>  fetch_contract_by_number(uint32_t num)const;
         const signed_transaction   get_recent_transaction( const transaction_id_type& trx_id )const;
         std::vector<block_id_type> get_block_ids_on_fork(block_id_type head_of_fork) const;

         chain_id_type gamebank_chain_id;
         ...
         */
         ...
         const crowdfunding_object&  get_crowdfunding(const account_name_type& originator, const string& permlink)const;
         const crowdfunding_object*  find_crowdfunding(const account_name_type& originator, const string& permlink)const;
         ...
         void confirm_block( const signed_block& blk, uint32_t skip = skip_nothing);
      ...
      public:
          ...
          *
          * Passing slot_num == 0 returns GAMEBANK_NULL_WITNESS
          */
         ...
         /** @return the gbd created and deposited to_account, may return GBC if there is no median feed */
         std::pair< asset, asset > create_gbd( const account_object& to_account, asset gbc, bool to_reward_balance=false );
         asset create_vesting( const account_object& to_account, asset gbc, bool to_reward_balance=false );
         void adjust_total_payout( const comment_object& a, const asset& gbd, const asset& curator_gbd_value, const asset& beneficiary_value );
         ...
         /** this updates the votes for all witnesses as a result of account GBS changing */
         ...
         /**
          * Helper method to return the current gbd value of a given amount of
          * GBC.  Return 0 GBD if there isn't a current_median_history
          */
         asset to_gbd( const asset& gbc )const;
         asset to_gbc( const asset& gbd )const;
         ...
         void init_schema();
         void init_genesis(uint64_t initial_supply = GAMEBANK_INIT_SUPPLY );
         ...
         void apply_pre_genesis_patches( void );
   ...
   protected:
      ...
      private:
         ...
         void update_confirm_witness(const witness_object& confirm_witness, const signed_block& new_block);
         ...
         fc::time_point_sec            _hardfork_times[ GAMEBANK_NUM_HARDFORKS + 1 ];
         ...
         uint32_t                      _next_available_nai = GB_MIN_NON_RESERVED_NAI;
