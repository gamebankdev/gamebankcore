...
#include <gamebank/chain/buffer_type.hpp>
...
using gamebank::protocol::chain_id_type;
...
class crowdfunding_object;
class crowdfunding_content_object;
...
typedef oid< crowdfunding_object                    > crowdfunding_id_type;
typedef oid< crowdfunding_content_object            > crowdfunding_content_id_type;
typedef oid< crowdfunding_invest_object             > crowdfunding_invest_id_type;
typedef oid< nonfungible_fund_object				> nonfungible_fund_id_type;
typedef oid< nonfungible_fund_on_sale_object		> nonfungible_fund_on_sale_id_type;
...
{
   class variant;
   inline void to_variant( const gamebank::chain::shared_string& s, variant& var )
   {
      var = fc::string( gamebank::chain::to_string( s ) );
   }

   inline void from_variant( const variant& var, gamebank::chain::shared_string& s )
   {
   ...
   namespace raw {
      ...
      template< typename T >
      inline T unpack_from_vector( const gamebank::chain::buffer_type& s )
      { try  {
         T tmp;
         if( s.size() ) {
...
FC_REFLECT_ENUM( gamebank::chain::object_type,
                 (dynamic_global_property_object_type)
                 (account_object_type)
                 (account_authority_object_type)
                 (witness_object_type)
...
#ifndef ENABLE_STD_ALLOCATOR
FC_REFLECT_TYPENAME( gamebank::chain::shared_string )
#endif

FC_REFLECT_ENUM( gamebank::chain::bandwidth_type, (post)(forum)(market) )
