...
{
   /*
   * if new account created by initminer, and the initminer will give a certain amount of vest shares to the new account by delegation
   */
   ...
   if (creator.name == GAMEBANK_INIT_MINER_NAME)
   {
...
void contract_deploy_evaluator::do_apply(const contract_deploy_operation& op)
{
	try {
		...
		if (current_opcount > 0) {
			note.update_bandwidth = current_opcount;
			_db.notify_update_bandwidth(note);
		}
