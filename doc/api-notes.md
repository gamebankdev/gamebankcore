...
We recommend using the first of these methods to secure the API by binding to localhost, as follows:
...
The `gamebankd` process includes username/password based authentication to individual API's.
...
Now let's protect the `hello_api_api` API to only be usable by user `tom` with password `supersecret`.  Modify the `config.ini` file to make
...
`api-user` definition to define the access policy:
    ...
    api-user = {"username" : "tom", "password_hash_b64" : "T2k/wMBB9BKyv7X+ngghL+MaoubEuFb6GWvF3qQ9NU0=", "password_salt_b64" : "HqK9mAQCkWU=", "allowed_apis" : ["hello_api_api"]}
...
Username/password access is only available through websockets, since login is inherently *stateful*.  We can use the `wscat` program to demonstrate the login functionality:
    ...
    < (error)
    {"jsonrpc": "2.0", "params": ["login_api", "login", ["tom", "supersecret"]], "id":3, "method":"call"}
    < (ok)
