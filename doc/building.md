...
Gamebank requires Boost 1.58 (as in Ubuntu 16.04) and works with versions up to 1.60 (including).
So building Gamebank on Ubuntu 14.04 requires downloading and installing a more recent
version of Boost.
...
Ubuntu 14.04) when compiling in C++11 mode (which Gamebank does).
...
Then the instructions are the same as for gamebank:
