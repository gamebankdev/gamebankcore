...
{
   gamebank::protocol::transaction     tx;
   std::string                      wif;
};
...
struct tx_signing_result
{
   ...
   gamebank::protocol::public_key_type key;
   gamebank::protocol::signature_type  sig;
};
...
int main(int argc, char** argv, char** envp)
{
   ...
   while( std::cin )
   {
      ...
      try
      {
         ...
         auto priv_key = gamebank::utilities::wif_to_key( sreq.wif );

         if(priv_key)
         {
            sres.sig = priv_key->sign_compact( sres.sig_digest );
            sres.key = gamebank::protocol::public_key_type( priv_key->get_public_key() );
