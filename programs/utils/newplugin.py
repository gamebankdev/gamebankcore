...
target_link_libraries( {plugin_provider}_{plugin_name} gamebank_app gamebank_chain gamebank_protocol )
...
{{
   public:
      {plugin_name}_api( const gamebank::app::api_context& ctx );
...
#include <gamebank/app/plugin.hpp>
...
class {plugin_name}_plugin : public gamebank::app::plugin
{{
   public:
      {plugin_name}_plugin( gamebank::app::application* app );
...
"{plugin_name}_api.cpp" :
"""
#include <gamebank/app/api_context.hpp>
#include <gamebank/app/application.hpp>
...
{{
   public:
      {plugin_name}_api_impl( gamebank::app::application& _app );

      std::shared_ptr< {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin > get_plugin();

      gamebank::app::application& app;
}};

{plugin_name}_api_impl::{plugin_name}_api_impl( gamebank::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const gamebank::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( gamebank::app::application& app );
      ...
      gamebank::app::application& _app;
      boost::signals2::scoped_connection _applied_block_conn;
}};

{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( gamebank::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( gamebank::app::application* app )
   : plugin(app)
{{
...
GAMEBANK_DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
...
def main(argv):
    ...
    parser.add_argument("provider", help="Name of plugin provider")
