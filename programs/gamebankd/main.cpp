...
#include <gamebank/manifest/plugins.hpp>

#include <gamebank/protocol/types.hpp>
#include <gamebank/protocol/version.hpp>

#include <gamebank/utilities/logging_config.hpp>
...
#include <gamebank/plugins/webserver/webserver_plugin.hpp>
...
{
   static string v_str =
      "gamebank_blockchain_version: " + fc::string( GAMEBANK_BLOCKCHAIN_VERSION ) + "\n" +
      "gamebank_git_revision:       " + fc::string( gamebank::utilities::git_revision_sha ) + "\n" +
...
#else
      std::cerr << "------------------------------------------------------\n\n";
      std::cerr << "            STARTING GAMEBANK NETWORK\n\n";
      std::cerr << "------------------------------------------------------\n";
      std::cerr << "initminer public key: " << GAMEBANK_INIT_PUBLIC_KEY_STR << "\n";
      std::cerr << "chain id: " << std::string( GAMEBANK_CHAIN_ID ) << "\n";
      std::cerr << "blockchain version: " << fc::string( GAMEBANK_BLOCKCHAIN_VERSION ) << "\n";
...
int main( int argc, char** argv )
{
   try
   {
      ...
      gamebank::utilities::set_logging_program_options( options );
      options.add_options()
      ...
      gamebank::plugins::register_plugins();

      appbase::app().set_version_string( version_string() );
      appbase::app().set_app_name( "gamebankd" );
      ...
      appbase::app().set_default_plugins<
         ...
         gamebank::plugins::account_by_key::account_by_key_api_plugin,
         gamebank::plugins::condenser_api::condenser_api_plugin >();
      ...
      bool initialized = appbase::app().initialize<
            ...
            gamebank::plugins::webserver::webserver_plugin >
            ( argc, argv );
      ...
      try
      {
         fc::optional< fc::logging_config > logging_config = gamebank::utilities::load_logging_config( args, appbase::app().data_dir() );
         if( logging_config )
